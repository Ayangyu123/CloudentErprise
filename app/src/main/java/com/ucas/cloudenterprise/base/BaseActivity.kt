            ...
            response?.let {
                ...
                when (code) {
                    ...
                    REQUEST_ACCESS_TOKEN_FAIL_CODE ->//access_token 失效
                    {
                        ...
                        NetRequest(GET_REFRESH_ACCESS_TOKEN + REFRESH_TOKEN + USER_ID,
                            NET_GET,
                            null,
                            NetTag!!,
                            object : BaseActivity.OnNetCallback {
                                override fun OnNetPostSucces(
                                    ...
                                    MyApplication.context.getSharedPreferences(
                                        PREFERENCE__NAME__FOR_PREFERENCE,
                                        Context.MODE_PRIVATE
                                    ).edit().putString("access_token", ACCESS_TOKEN).commit()

                                    AddToken(ACCESS_TOKEN)
                                    Log.e(
                                        "base",
                                        "token  is ${OkGo.getInstance().commonHeaders.get("Authorization")}"
                                    )
...
//        }
        when (RequestMethod) {
            ...
            NET_GET -> {
                var stringA = StringBuffer()
                OkGo.get<String>(url).tag(tag).execute(netcallback)
            }
            NET_POST -> {
                OkGo.post<String>(url).upJson(
                    json
//                        .toString().replace("\\","")
                ).tag(tag).execute(netcallback)
            }
            NET_PUT -> {
                OkGo.put<String>(url).upJson(JSONObject(paramsjson as Map<String, Any>)).tag(tag)
                    .execute(netcallback)
            }
        }
